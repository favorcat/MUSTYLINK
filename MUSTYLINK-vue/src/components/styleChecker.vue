<template>
  <div id="wrapper">
    ...
    <a class="result"> {{ res.results }} </a>
    <div class="styleBox-wrapper">
      <div class="styleBox" v-for="(v, i) in styleList.styles" :key="i" @click="goMusinsa(v.url)">
        <img :href="v.url" :src="v.src" width= "230" >
      </div>
    </div>
  </div>
</template>
...
const styleList_k = ['아메리칸 캐주얼','캐주얼', '시크', '댄디', '포멀', '걸리시', '골프', '레트로', '로맨틱', '스포츠', '스트릿']
const styleList_e = ['americancasual', 'casual', 'chic', 'dandy', 'formal', 'girlish', 'golf', 'retro', 'romantic', 'sports', 'street']
const res = reactive({
  results: ''
})
...
async function predict(element, flip = false) {
  ...
  let maxStyle = 0;
  let idx = 0;
  prediction.map(predicitonElement => {
    const perc = (predicitonElement.probability.toFixed(2) * 100).toFixed(0)
    if (maxStyle < perc) {
        maxStyle = perc
        idx = i
    }
  ...
  res.results = styleList_k[idx]
  axios.get(`./json/${styleList_e[idx]}.json`)
    .then((res) => {
        styleList.styles = res.data;
    });
  console.log(styleList.styles[0])
//   this.$store.state.title = styleList_s[idx]
//   state.classes = prediction.map(predicitonElement => {
//       arr[i] = (predicitonElement.probability.toFixed(2) * 100).toFixed(0)
//       i += 1
//     return predicitonElement.className + ": " + (predicitonElement.probability.toFixed(2) * 100).toFixed(0) + '%'
//   })
//   const idx = arr.indexOf(Math.max.apply(null,arr))
//   console.log(prediction[idx].className)
}
